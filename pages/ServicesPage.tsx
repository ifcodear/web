  ...
  const capabilities = [
    {
      ...
      link: Page.Engineering as Page | null,
      icon: (
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {capabilities.map((c, i) => (
            <div
              key={i}
              onClick={() => { if (c.link) setCurrentPage(c.link); }}
              className={[
                'group p-10 bg-surface-dark border border-white/10 rounded-3xl hover:border-primary/50 transition-all shadow-xl',
            ...
            >
              ...
              <div className="mt-8 pt-6 border-t border-white/5">
                <span
                  className={[
                    'flex items-center gap-2 text-primary font-black text-sm group-hover:gap-4 transition-all',
                    c.link ? '' : 'opacity-40'
                  ].join(' ')}
                >
                  Explorar enfoque
